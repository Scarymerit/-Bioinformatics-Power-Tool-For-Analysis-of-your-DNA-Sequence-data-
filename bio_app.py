...
from Bio.Align.Applications import ClustalwCommandlineimport streamlit as st
import os
import io
import ssl
from Bio import Entrez, SeqIO, Align
...
if input_method == "Enter Accession ID":
    ...
    if st.sidebar.button("Fetch and Process"):
        ...
        if record:
            records = [record]  # Store as a list of consistency
...
def perform_msa(records):
    ...
    try:
        # Check if clustalw executable is available
        subprocess.run(["clustalw", "-version"], check=True, capture_output=True)

        # Run ClustalW
        cline = ClustalwCommandline("clustalw", infile=temp_fasta_file, outfile=output_aln_file)
